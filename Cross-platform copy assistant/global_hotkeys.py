...
from pallet_icon import clipboard_get_time_notify2  # 调用本地函数
...
def main():
    hotkey = Hotkey()
    hotkey.reg(key=(win32con.MOD_CONTROL, win32con.VK_F1), func=clipboard_get_time_notify2)  # alt home键 开始
    hotkey.start()  # 启动热键主线程

    print(f"当前总线程数量:{active_count()}")
    print('当前线程列表:', enumerate())
    print('热键注册初始化完毕，按组合键CTRL+F1开始运行')
